...
import { getAxiosCancel } from './AxiosCancel'
...
export class VAxios {
  ...
  private setupInterceptors() {
    ...
    const axiosCanceler = getAxiosCancel()

    // 请求配置处理
    this.instance.interceptors.request.use((config: InternalAxiosRequestConfig) => {
      const {
        headers: { notCacheRequest },
      } = config

      // 是否将 cancel 加入 map
      const ignoreRepeat = notCacheRequest ?? this.options.requestOptions?.ignoreRepeatRequest
      ...
      this.instance
        ...
        .catch((e: Error | AxiosError) => {
          ...
          if (axios.isAxiosError(e)) {
            ...
            console.log('当前的错误信息', e)
            const axiosCancelerInstance = getAxiosCancel()
            switch (status) {
              case 401:
                localStorage.removeItem(TOKEN_NAME)
                // 清空所有请求
                axiosCancelerInstance.removeAllPending()
                showNotify({
                  type: 'danger',
                  message: '登录已过期，请重新登录',
                })
